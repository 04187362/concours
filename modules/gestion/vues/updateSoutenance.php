...
<?php
	...
	if(!empty($_POST['id_paye']) && !empty($_POST['etudiant']) && !empty($_POST['filiere']) && !empty($_POST['niveauetude']) && !empty($_POST['cycle']))
		{
			...
			if(existanceGestion($requete3,$param3)==0){
					...
					$count = $soutenance->modifiersoutenance();

					if($count>0){

						echo '<div style="text-align:center" class="text-mint"><b>Soutenance modifié avec succès.</b></div>';
