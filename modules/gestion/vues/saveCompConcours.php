<?php
    ...
    include("globale/verification.php");
		...
		if(!empty($id_an) && !empty($_POST['matiere']) && !empty($_POST['candidat']) && !empty($_POST['note']))
		{
			...
			if($note >= 0 && $note <= 20){
				...
				$requete4 = "SELECT * FROM composer_concours WHERE id_candidat =? AND id_matiere=? AND id_anneeAc=?";
				...
				if(existanceGestion($requete4, $param4)==0){
						...
						$count = $composition->ajouterCompositionConcours();
						...
						echo '<div style="text-align:center; color:orange">L\'candidat <span style="color:yellow">'.$nom_etu.' '.$prenom_etu.'</span> a déjà composé la matière de <span style="color:yellow">'.$lib_matiere.'</span>.</div></div>';
