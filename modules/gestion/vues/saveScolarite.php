...
<?php
	...
	if(!empty($_POST['etudiant']) && !empty($_POST['filiere']) && !empty($_POST['niveauetude']) && !empty($_POST['frais']))
		{
            ...
            $requete3 = "SELECT * FROM payement WHERE id_etu=? AND id_annee=? AND type_payement='scolarite'";
			...
			if(existanceGestion($requete3,$param3)==0){
					...
					$count = $scolarite->ajouterScolarite();

					if($count>0){

						echo '<div style="text-align:center" class="text-mint"><b>Scolarite enregistré avec succès</b></div>';
					...
					echo '<div style="text-align:center; color:orange">L\'etudiant <span style="color:yellow">'.$nom_etu.' '.$prenom_etu.'</span> a déjà payé ses frais de scolarité pour l\'année '.$libelle_anneeAc.'.</div></div>';
			}						
